...
*What declining leaves:* the values stay under the old keys in every scope file, unreachable from the renamed module but **not deleted** — nothing on the rename path removes them, and renaming back exactly reaches them again, since the keys are plain strings and `remap_all_scopes` is symmetric. **`crate::gui::GuiApp::config_cleanup` does reap them**, however: it removes every `(author, name, variable)` no loaded module declares, and post-rename no module declares those. Both facts are in the dialog text; see `superdoc/features/settings-gui.md` for the wording and the reasoning behind it.
